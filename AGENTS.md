...
Linux 好打注音的本機修正：第十個音節完成時送出最前方完整詞段；句中可用方向下鍵開候選，支援 Fcitx preedit click 的客戶端也可點字改選；切換輸入法時先提交可見組字。候選窗開啟後，Enter 應先套用反白候選並保留組字，下一次 Enter 才送出整句。C++ CTest 2/2 與隔離 Ubuntu 24.04 X11/GTK3 的長句擠出、切換輸入法、方向鍵確認候選與滑鼠點候選實際輸入案例已通過。這些結果不等於 GNOME Wayland 或所有 App 驗收；系統目前安裝的 `fix2` 套件尚未包含 Enter 候選修正。
