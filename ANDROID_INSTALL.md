...
1. 預設的**好打注音**可連續輸入每個字的注音與聲調，不必逐字選定；沒有聲調的一聲字
   以 **空白**結束音節。鍵盤會依前後文組句，已組好的中文字會隨打字顯示在 App 的文字欄位。
2. 鍵盤最上方的組字列保留最近 **9 個**已完成的音節。點其中任何一個字，例如點「玩」，
   就能選「完」等同音候選，並直接修正文字欄位中的對應字；兩端的 **▲／▼**、左右滑動與候選開啟時的空白可換頁。
3. 試打「請假要去哪裡玩呢去海邊」：繼續輸入到「海邊」時，「請假」已移出可修改的組字列，
   上方保留「要去哪裡玩呢去海邊」。整句仍顯示在文字欄位；切換 App 或輸入法後，已輸入的中文字不會因組字列消失而遺失。
4. 在搜尋欄按 **搜尋**，或在其他欄位按 **Enter／前往／傳送**，按一次就會執行該欄位的動作，
   不必先按一次清除組字列。
5. 若偏好逐字選字，從 **琦琦注音 App → 3. 調整設定 → 注音模式**選擇「傳統注音」。

<img src="docs/images/keykey-android-smart-search.png" alt="Android 搜尋欄已顯示請假要去哪裡玩呢去海邊，鍵盤組字列保留要去哪裡玩呢去海邊" width="360">

*Android 實機畫面：文字欄位已有整句，上方組字列仍可修改最近的字；右下角是「搜尋」鍵。*

<img src="docs/images/keykey-android-middle-candidate.png" alt="Android 琦琦注音點選組字列中間的玩字後，顯示玩、完等候選字" width="360">

*點組字列中間的「玩」，可在候選列改選「完」等字。*
...
| **↵／搜尋／前往／傳送** | 按一次即可依目前欄位換行或執行對應動作；已顯示在文字欄位的中文字不會重複輸入。 |
...
打開 **琦琦注音 App → 3. 調整設定**，可以切換好打注音／傳統注音、管理自訂詞與重設學習紀錄，
並調整按鍵震動、按鍵預覽、候選字底色、實體鍵盤的浮動候選字窗，以及直式與橫式的螢幕鍵盤高度。
注音模式預設為好打注音；不確定怎麼選時，先保留預設值。

<img src="docs/images/keykey-android-settings-top.png" alt="Android 琦琦注音設定頁上半部，顯示注音模式、自訂詞、學習紀錄、震動、按鍵預覽及候選字底色" width="360">

*Android 實機畫面：設定頁上半部可選注音模式，並調整自訂詞、學習紀錄、按鍵回饋與候選字顏色。*

<img src="docs/images/keykey-android-settings-bottom.png" alt="Android 琦琦注音設定頁下半部，顯示實體鍵盤浮動候選字窗、直式與橫式虛擬鍵盤高度及關聯詞詞庫" width="360">

*往下滑可設定實體鍵盤浮動候選字窗，並分別調整直式、橫式螢幕鍵盤的高度。*
