...
> 本指南適用於 **1.3.0**。請在正式發布頁確認版本與安裝檔名後下載。下圖是操作示意；頁面排列可能隨發布調整。

本文的輸入法選單、偏好設定與候選字實拍取自本機 **1.3.0** 建置；不同 macOS 版本的畫面外觀可能略有差異。
...
1. 打開 [琦琦輸入法的正式桌面版下載頁](https://github.com/polobread/KeyKey/releases/latest)，確認頁面顯示 **v1.3.0**。若看到很多檔案，往下找到 **Assets**，必要時按一下展開。
2. 點選 **`chichi77-KeyKey-1.3.0-macos-arm64.pkg.zip`**。請確認檔名中的版本與上方發布版本一致。
...
*圖 1：下載檔案的操作示意；實際發布頁的排列可能不同，請以 v1.3.0 與檔名為準。*
...
3. 在琦琦輸入法選單確認目前使用 **好打注音**。用你平常的注音鍵位連續輸入 **ㄊㄞˊ ㄅㄟˇ**，畫面會組成「台北」，不必先確定「台」再打「北」。
4. 按 **Return** 確定整段文字。若自動選字不合預期，按 **空白鍵**叫出游標位置的候選字，再用候選字旁的數字鍵或滑鼠更正。

![文件中開啟的琦琦輸入法直式候選字窗，第二個候選字反白](docs/images/keykey-macos-v130-candidates.png)

*圖 3：1.3.0 在文件中選字的實拍。候選字左邊的數字是選字鍵，紫色列是目前反白的字；底部的 `1/4` 表示目前是第 1 頁、共 4 頁。*

## 日常使用

### 注音與選字

選好琦琦輸入法後，在想打字的 App 點一下文字欄位。預設的 **好打注音**會把每個完整音節加入同一段組字文字，依詞頻自動選字；例如連續輸入「ㄐㄧㄣ ㄊㄧㄢ ㄅㄟˋ ㄑㄧㄤˊ ㄓˋ ㄕㄥ ㄐㄧˊ ㄉㄠˋ」，可直接得到「今天被強制升級到」，最後再按 Return 送出整段。

一聲沒有聲調符號，輸入完整讀音後按 **空白鍵**完成該音節；二、三、四、輕聲則在輸入聲調時完成。組字期間按空白鍵會叫出目前位置的候選字，選字後仍可繼續輸入後面的音節。
...
| **Return** | 候選字清單開啟時選取反白字；沒有候選窗時確定整段組字文字。 |
| **Esc** | 關閉目前的候選字或聯想詞清單。 |
...
- 琦琦輸入法的同一個選單內可選 **好打注音、傳統注音、倉頡、簡易**。想連續輸入整句時使用「好打注音」；若習慣每個字立即顯示候選窗，再改用「傳統注音」。
- 需要特殊符號時，從琦琦輸入法的選單點 **符號表**，在表內選取符號。

![琦琦輸入法選單顯示好打注音已選取，並列出其他輸入模式、符號表及偏好設定](docs/images/keykey-macos-v130-input-menu.png)

*圖 4：1.3.0 的輸入法選單；勾選記號表示目前正在使用「好打注音」。*

### 自訂詞

使用好打注音時，可從右上角琦琦輸入法選單點 **編輯自訂詞…**，新增、刪除或修改
個人詞彙及讀音。也可以在組字文字中按住 Shift 搭配左右方向鍵選取最多六個字，再按
Return 將選取內容加入自訂詞。匯入與匯出功能位於 **偏好設定… → 詞彙管理**。

如果長期選字後的結果變得不符合目前習慣，可從同一選單點 **重設好打注音學習紀錄…**。
確認後只會清除輸入法從候選選擇與相鄰詞累積的學習資料，已新增的自訂詞仍會保留。
...
![文件、偏好設定與琦琦輸入法選單同時顯示的 macOS 畫面](docs/images/keykey-macos-v130-preferences-overview.png)

*圖 5：從右上角的琦琦輸入法選單可進入「偏好設定…」；這裡顯示開啟後的「一般」頁。*

![琦琦輸入法偏好設定的一般頁，顯示候選窗比例、直式與橫式候選窗選項](docs/images/keykey-macos-v130-preferences-general.png)

*圖 6：「一般」頁近照；在這裡調整候選窗比例、直排或橫排，以及候選窗配色。*
...
| 好打注音輸入一聲後沒有進入下一字 | 一聲沒有聲調符號，輸入完整讀音後按空白鍵完成該音節。若空白鍵叫出候選窗，先選字或按 Esc 關閉再繼續。 |
| 每個字都立刻出現候選窗，無法整句連打 | 從右上角琦琦輸入法選單改選「好打注音」；「傳統注音」本來就是逐字候選模式。 |
