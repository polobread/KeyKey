...
打開 **開始 → 設定 → 系統 → 關於**，在「裝置規格」查看「系統類型」。目前套件適用於 **Windows 11、64 位元 x64 電腦**；ARM64 電腦尚未驗證，也不在此套件支援範圍。安裝時需要同意 Windows 的管理員權限提示。
...
仍無法使用時，請記下 Windows 版本、系統類型（x64 或 ARM64）、下載的檔名、`Win + Space` 清單是否看到琦琦輸入法，以及在記事本試打的畫面。
