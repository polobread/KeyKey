...
App 內有可選擇的「支持開發」一次性購買，**不購買也能完整使用輸入功能**。GitHub 上的 iOS 模擬器測試包不能安裝到 iPhone 或 iPad。
...
3. 點注音鍵與聲調，例如 **ㄑㄧˊ**。候選字出現後，直接點要用的字；候選列左右兩端的箭頭可以翻頁。
4. 再輸入下一個字。這一版是**逐字選字**；選好一個字後，鍵盤可能顯示常接在後面的「關聯詞」供你點選。

<img src="StoreAssets/Sources/ios-notes-qi.png" alt="琦琦注音在 iPhone 文字欄位輸入注音並顯示候選字的實際畫面" width="360">

*圖 2：專案既有的 iPhone 實際輸入截圖。上方是候選字列，下方是五排注音鍵盤；截圖使用專案的測試用記事本，並非 iPhone 內建「備忘錄」。*
...
| **符**、**☺** | 開啟符號或表情符號列表。 |
| **設** | 選擇關聯詞詞庫、調整候選字底色等鍵盤設定。 |
| **空白**、**⌫**、**↵** | 空白／翻候選頁、刪除、確認或換行；實際動作會依正在組字或所用 App 的欄位而變。 |

詞庫設定請在**正在使用的琦琦注音鍵盤**點「設」。下圖是實際設定畫面；可勾選想要的關聯詞庫，不確定時先保留預設值。

<img src="StoreAssets/Sources/ios-phone-dictionaries.png" alt="琦琦注音鍵盤的關聯詞詞庫設定實際畫面" width="360">

*圖 3：鍵盤內的詞庫設定。這和下方「實體鍵盤編輯器」的詞庫設定分開保存。*
...
*圖 4：實體鍵盤編輯器畫面；圖中的連線狀態、候選字和按鈕狀態會依你的操作改變。編輯器只能在琦琦注音 App 開啟時使用。*
