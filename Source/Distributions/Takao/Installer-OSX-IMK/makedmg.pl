...
$ENV{WRAPPER_NAME} = "chichi77 KeyKey.app";
...
@hdiutilOutput = `hdiutil create -sectors $imageSize -volname "chichi77 KeyKey for Mac OS X Leopard.localized" -attach -fs HFS+ "$build_products_directory/chichi77 KeyKey.dmg"`;
...
system("rm -f \"$build_products_directory/chichi77-OSX-Leopard-" . $version . ".dmg\"");
mySystem("hdiutil convert \"$build_products_directory/chichi77 KeyKey.dmg\" -format UDZO -imagekey zlib-level=9 -o \"$build_products_directory/chichi77-OSX-Leopard-" .  $version . ".dmg\"");
system("rm -f \"$build_products_directory/chichi77 KeyKey.dmg\"");
print "-----Done building.\n";
system("rm openUp");
