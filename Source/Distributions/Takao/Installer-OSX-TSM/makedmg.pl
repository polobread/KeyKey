...
$ENV{"BUILT_PRODUCTS_DIR"} = "../../../build/Release/";

$ENV{WRAPPER_NAME} = "chichi77 KeyKey.bundle";
...
$project_target_name = "Takao (TSM Component)";
...
#Make clean
system "xcodebuild -sdk macosx10.4 -project ../../../Takao.xcodeproj -configuration Release clean";
#Make install
mySystem "xcodebuild -sdk macosx10.4 -project ../../../Takao.xcodeproj -configuration Release -target \"" . $project_target_name . "\" build";
...
system("rm -f \"$build_products_directory/chichi77 KeyKey.dmg\"");
@hdiutilOutput = `hdiutil create -sectors $imageSize -volname "chichi77 KeyKey for Mac OS X Tiger.localized" -attach -fs HFS+ "$build_products_directory/chichi77 KeyKey.dmg"`;
...
on process_disk_image(volumeName, artPath)
       tell application "Finder"
               tell disk (volumeName as string)
                       ...
                       open
                       set volumeName to "chichi77 KeyKey"
...
system("rm -f \"$build_products_directory/chichi77-OSX-Tiger-" . $version . ".dmg\"");
mySystem("hdiutil convert \"$build_products_directory/chichi77 KeyKey.dmg\" -format UDZO -imagekey zlib-level=9 -o \"$build_products_directory/chichi77-OSX-Tiger-" .  $version . ".dmg\"");
system("rm -f \"$build_products_directory/chichi77 KeyKey.dmg\"");
print "-----Done building.\n";
system("rm openUp");
