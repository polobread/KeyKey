    ...
    , m_userDatabase(0)
{
    ...
    // sqlite3_open() does not read the file, so a database it cannot parse --
    // one the 1.1 line encrypted with the SQLite Encryption Extension -- only
    // fails once a statement runs. Set it aside rather than write nothing for
    // the rest of the session.
    if (!m_userDatabase->hasTable("data")) {
        delete m_userDatabase;
        rename(userDatabasePath.c_str(), (userDatabasePath + ".unreadable").c_str());
