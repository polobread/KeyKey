...
| F. 英數與符號 | 英文 Shift、數字兩面、90 符號、90 emoji、全半形標點 | 英文 Shift 後 `; , . /` 顯示並送出 `: < > ?`；Shift／模式狀態正確；頁面可完整選取；禁用鍵淡化且不可點 |
| G. 輸入欄位 | Debug host 的一般、Email、URL、電話、整數、小數、日期時間、密碼、姓名、地址、搜尋、長文字、ASCII、ASCII 數字 | URL 起始即可用完整ㄅ／英／數；其餘受限欄位保留原起始模式，但 MODE 可按且第一次按下即解鎖完整三模式與功能，同一欄位重顯不再鎖回；密碼／電話被 iOS 換成系統鍵盤時記為預期系統限制 |
