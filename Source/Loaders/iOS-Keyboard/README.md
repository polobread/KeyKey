...
`KeyKey.db` Bigram 模型連續組句，輸入下一個音節前不必先選字；Enter 確定整句，候選
選取只覆寫目前最後一個音節。傳統注音維持原本逐字選字與關聯詞流程。
...
- **inline 組字**：專案最低 iOS 17，而 `UITextDocumentProxy.setMarkedText` 自 iOS 13
  可用。好打注音的整句中文字加上尚未完成的注音會共同顯示為 marked text；傳統注音
  則只標記目前讀音，選字時以候選字取代讀音。
