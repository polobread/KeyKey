...
![琦琦注音在 Android 記事本中以實體鍵盤顯示 ㄑㄧˊ 浮動候選](../../../StoreAssets/Sources/android-notes-floating-qi.png)
...
- 橫式：沿用相同的候選列、四排 11 等寬鍵與功能列，100% 時內容高度為 155dp，並使用
  較小字級避免裁切。設定頁可把直式與橫式虛擬鍵盤分別調成 50%–200%。
...
欄位限制只改變觸控版面與觸控按鍵；外接實體鍵盤維持 Windows／macOS 版的輸入行為，
不依 `inputType` 擋掉實體字元或快捷鍵。這能保留桌面鍵盤的完整輸入與修飾鍵習慣；
`inputType` 本身只是給 IME 的提示，欄位是否接受字元仍由 App 驗證。

## App 設定

App 首頁會避開狀態列與前相機挖孔，並提供第三個「調整設定」按鈕。設定頁的震動
回饋可在 0–0.1 秒間以 1 毫秒為單位調整；0 表示關閉。舊版的 0.15 與 0.2 秒設定
升級後會限制為 0.1 秒。變更後會立即套用到螢幕鍵盤的按鍵、候選字及翻頁按鈕。

「虛擬鍵盤高度」提供獨立的直式與橫式拉桿，兩者都能以 1% 為單位在 50%–200% 間
調整，預設為 100%。比例只調整觸控鍵盤內容；底部系統安全區、一般實體鍵盤候選列及
浮動候選字窗不受影響。
