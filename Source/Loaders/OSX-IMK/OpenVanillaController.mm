...
- (void)activateServer:(id)sender
{
	...
	// syncLoaderConfig only reaches the loader's own plist. Preferences writes
	// the modules' plists, and without this a change there does not take effect
	// until the input method is switched.
...
- (void)preferenceAction:(id)sender
{
	...
	if (![[NSWorkspace sharedWorkspace] openFile:preferencePath]) {
		[[NSWorkspace sharedWorkspace] launchAppWithBundleIdentifier:@"io.github.polobread.inputmethod.chichi77.Preferences" options:NSWorkspaceLaunchDefault additionalEventParamDescriptor:nil launchIdentifier:nil];
	}
	[self _resetUI];	
}
...
- (NSMenu *)menu
{
    ...
    for (iter = idNamePairs.begin(); iter != idNamePairs.end(); ++iter) {
		...
		if (identifier == "AssociatedPhrase" || identifier == "OneKey" || identifier == "Evaluator") {
			if (![OpenVanillaLoader sharedLoader]->isAroundFilterActivated(identifier)) {
				[OpenVanillaLoader sharedLoader]->toggleAroundFilter(identifier);
			}
			continue;
	...
	[aboutMenuItem setTitle:LFLSTR(@"About chichi77 KeyKey")];
