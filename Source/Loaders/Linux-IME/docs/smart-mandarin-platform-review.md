...
- Linux 本輪保留既有「明確候選改選」的學習時機；iOS／Android 還會在確認整句時呼叫 `learnConfirmedComposition`。本輪沒有改寫各平台的模型分數或宣稱所有候選排序相同。
...
- 本輪沒有 macOS／Windows／iOS／Android SDK 或實機驗證；GNOME 原生 Wayland 及其他應用程式的文字宿主行為也尚未實測。原始碼、staged addon、安裝包和系統已安裝版本分開記錄。
