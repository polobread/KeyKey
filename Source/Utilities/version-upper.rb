...
def replace_extra_module_wxs(filename, version)
  ...
  while line = file.gets
    ...
    if line =~ /<Product/
      line.gsub!(/Id=".+?"/, "Id=\"#{product_id}\"")
      line.gsub!(/Name=".+?"/, "Name=\"琦琦輸入法外掛安裝包 #{version_parts[0]}.#{version_parts[1]} 正式版 (build #{version_parts[2]})\"")
      line.gsub!(/Version=".+?"/, "Version=\"#{version}\"")
    elsif line =~ /<Package/
      line.gsub!(/Id=".+?"/, "Id=\"#{package_id}\"")
    elsif line =~ /UpgradeVersion/
...
def replace_panel_about_cs(filename, version)
  version_parts = version.split(/\./)    
  content = IO.read(filename)
  content.gsub!(/Yahoo.+?\(build \d+\)/, "chichi77 KeyKey #{version_parts[0]}.#{version_parts[1]} (build #{version_parts[2]})")
  file = File.open(filename, "w")
  file.print content
  file.close
end
