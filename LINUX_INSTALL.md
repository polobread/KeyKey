...
`v1.2.9` 的 Linux 支援範圍是 **Ubuntu Desktop 24.04 LTS、GNOME Shell 46、Fcitx 5、amd64**。在「設定 → 關於」查看 Ubuntu 版本；打開終端機執行 `dpkg --print-architecture`，結果須為 `amd64`。首次安裝需要管理員密碼，並須登出、重新登入一次。其他 Ubuntu 版本、ARM64、IBus 與其他桌面環境尚未列入此版支援範圍。
