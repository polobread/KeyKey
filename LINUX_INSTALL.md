...
想自行編譯原始碼的使用者，請改看 [Linux `./configure` 編譯安裝與使用指南](LINUX_CONFIGURE_INSTALL.md)；這份文件以下使用已封裝的 `.deb` 安裝檔。

本指南使用與 macOS、Windows 相同的 **`v1.2.9`** 發布頁。下載前請確認頁面上有下列五個 Linux 檔案。三張實拍取自先前在 Ubuntu 24.04／GNOME 46 的測試畫面，用來示範設定與操作，尚不代表 1.2.9 的驗收結果。

## 安裝前確認

`v1.2.9` 的 Linux 支援範圍是 **Ubuntu Desktop 24.04 LTS、GNOME Shell 46、Fcitx 5、amd64**。在「設定 → 關於」查看 Ubuntu 版本；打開終端機執行 `dpkg --print-architecture`，結果須為 `amd64`。首次安裝需要管理員密碼，並須重新啟動 Ubuntu 一次。其他 Ubuntu 版本、ARM64、IBus 與其他桌面環境尚未列入此版支援範圍。

## 1. 下載 v1.2.9 套件

開啟 [琦琦輸入法 v1.2.9 發布頁](https://github.com/polobread/KeyKey/releases/tag/v1.2.9)，展開 **Assets**，將下列五個檔案放進同一個資料夾，例如「下載項目」中的 `keykey-v1.2.9`。若任一檔案尚未出現，先等套件備齊再安裝。
...
在已登入的 Ubuntu 桌面打開終端機，執行 `im-config`。在圖形精靈選 **fcitx5** 並完成設定。儲存手邊文件後，**重新啟動 Ubuntu**，再登入同一個帳號；只關閉終端機不會套用登入階段的輸入法設定。實測發現只登出圖形桌面時，其他登入工作階段可能保留舊的 IBus 環境；重啟可讓所有 App 使用 Fcitx。這是依照 [Fcitx 官方對 Ubuntu 的設定方式](https://fcitx-im.org/wiki/Setup_Fcitx_5)並加入實際 VM 測試的重啟步驟。
...
從 Fcitx 的輸入法選單選 **琦琦注音**。按 **Ctrl + Space** 可在 Fcitx 的英文鍵盤與琦琦注音間切換；切到琦琦注音後才能用注音組字。若按了仍只打出英文字母，先從 Fcitx 選單確認目前選的是琦琦注音，而非 Ubuntu 原有的注音或英文鍵盤。**Ctrl + Space** 沒有反應時，到 Fcitx 5 Configuration 的「全域選項」查看或調整切換快捷鍵。

琦琦注音本身另有一個**中文／英文模式**。已選到琦琦注音，卻仍直接輸出英文時，按 `Ctrl + \` 或短按一下 **Shift**，切回中文。這和 **Ctrl + Space** 的 Fcitx 輸入法切換是兩件事；**Shift + Space** 則只切換全形與半形。
...
| 按鍵 | 用法 |
| --- | --- |
| **Ctrl + Space** | 切換 Fcitx 的英文鍵盤與琦琦注音；若設定過其他快捷鍵，以「全域選項」顯示的為準。 |
| `Ctrl + \` 或短按 **Shift** | 已選到琦琦注音時，切換其中文／英文模式；`Ctrl + \` 可在琦琦注音設定中關閉。 |
| **Shift + Space** | 切換半形與全形，不會切換輸入法。 |
| **讀音後按 Space** | 例如按 `5`、`j`、`/` 輸入 ㄓㄨㄥ 後，開啟候選清單。 |
| **1–9** 或滑鼠點選 | 選取對應的候選字。 |
| **候選清單中的 Space／Page Down**、**Page Up** | 候選字有多頁時，往後或往前翻頁。 |
...
- 實體雙螢幕、熱插拔、所有 GNOME 主題與所有 App 都尚未逐一驗證；使用時若遇到問題，請依下方方式記錄環境與重現步驟。
...
| 只有「文字編輯器（琦琦注音）」可用，Firefox 或終端機只輸出英文 | 重新啟動 Ubuntu 後再開啟這些 App。在終端機執行 `printenv GTK_IM_MODULE QT_IM_MODULE XMODIFIERS`，三行應依序為 `fcitx`、`fcitx`、`@im=fcitx`；若仍顯示 `ibus`，重新執行 `im-config` 選 **fcitx5** 並重啟。 |
